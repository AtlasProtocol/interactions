...
let Lottery = function () {
    ...
    LocalContractStorage.defineMapProperty(this, 'DrawedAddressArrayMap');
    LocalContractStorage.defineMapProperty(this, 'ResultArrayMap');
    LocalContractStorage.defineProperty(this, 'DrawedCount');
    LocalContractStorage.defineProperty(this, 'Round');
    LocalContractStorage.defineProperty(this, 'ResultCount');
    LocalContractStorage.defineProperty(this, 'AllDrawedAddresses', {
        parse: function(text) {
            return JSON.parse(text)
        },
        stringify: function(o) {
...
Lottery.prototype.init = function () {
    ...
    this.Round = 0;
    this.ResultCount = 0;
    this.AllDrawedAddresses = [];
    return 'deploy ok'
};


Lottery.prototype.ini = function (addresses) {
    this._checkMasterAccount();
    let k = Math.random();
    let m = this.Round;
    let ii = 0;
    ...
    for (let i = 0; i < arr.length; ++i) {
        this._checkValidAddress(arr[i]);
        if (k <= m / n) {
            this.ValidAddressArrayMap.set(arr[i], true);
            m--;
            ii++;
        }
        n--;
...
Lottery.prototype.accept = function () {
    this.interact();
};

Lottery.prototype.draw = function(count) {
    this._checkMasterAccount();
    let ret = [];
    let hashInt = parseInt(Blockchain.transaction.hash, 16);
    let history = this.AllDrawedAddresses;
    for (let i = 0; i < count; i++) {
        let k = Math.random();
        let h = hashInt * k;
        let index = h % this.AddressCount;
        let address = this._getAddr(index);
        ret.push(address);
        history.push(address);
    }

    this.ResultCount++;
    this.ResultArrayMap.set(this.ResultCount, JSON.stringify(ret));
    this.AllDrawedAddresses = history;
};

Lottery.prototype.getResult = function (count) {
    if (count === undefined) {
        return this.ResultArrayMap.get(this.ResultCount)
    }
    else {
        return this.ResultArrayMap.get(count)
    }
};

Lottery.prototype.getHistory = function() {
    return this.AllDrawedAddresses
};

Lottery.prototype._getAddr = function (index) {
    let picker = index;
    let count = this.AddressCount;

    while (true) {
        let addr = this.AddressArrayMap.get(picker);

        if (this._isValidAddr(addr)) {
            this.DrawedAddressArrayMap.set(addr, true);
            this.DrawedCount += 1;
            return addr;
        }
        picker += 1;
        if (picker > this.AddressCount) {
            picker = 1;
        }
        count--;
        if (count < 0) {
...
Lottery.prototype._isValidAddr = function (addr) {
    if (addr == null) {
        return false
    }

    let remained = this.AddressCount - this.DrawedCount;

    let validAddr = this.ValidAddressArrayMap.get(addr);
    if (validAddr == null) {
        validAddr = false;
    }

    return !this.DrawedAddressArrayMap.get(addr) && ((remained > this.Round && !validAddr) || (remained <= this.Round && validAddr))
};

Lottery.prototype.setRound = function (round) {
    this._checkMasterAccount();
    this.Round = round;
};

Lottery.prototype.getRound = function () {
    return this.Round;
};
