...
let SingleStageVote = function () {
    SingleStageTIE.call(this);
    this.TIEType = 'SVM1.3.1';
};
...
(function () {
    ...
    SingleStageVote.prototype = new Super();
})();

SingleStageVote.prototype.getCount = function (address) {
    return this._caculateVote(address);
};

SingleStageVote.prototype._caculateVote = function (address) {
    let opts = this._getTIEOptions();
    let count = 0;
    let responses = (this.InteractArrayMap.get(address))['responses'];

    for (let index in responses) {
        if (responses.hasOwnProperty(index)) {
            let vote = JSON.parse(responses[index]['response'])['Answer'];
            count += parseInt(opts[vote]);
        }
    }
    return count
};

SingleStageVote.prototype.getTotalCount = function () {
    let count = 0;
    for (let i = 1; i < this.AddressCount + 1; i++) {

        count += this.getCount(this.AddressArrayMap.get(i));
    }

    return count
};

SingleStageVote.prototype._getTIEOptions = function () {
    for (let index in this.TIE) {
        if (this.TIE.hasOwnProperty(index) && this.TIE[index]['language'] === 'en') {
            return this.TIE[index]['options']
        }
    }
};

module.exports = SingleStageVote;
