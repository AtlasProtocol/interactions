    ...
    for (let i = 1; i < this.AddressCount + 1; i++) {
        ...
        for (let index in responses) {
            if (responses.hasOwnProperty(index)) {
                let answerObject = JSON.parse(responses[index]['response']);
                for (let index in answerObject['answers']) {
                    if (answerObject['answers'].hasOwnProperty(index)) {
                        if (ret[index] === undefined) {
                            ret[index] = [];
                        }
                        let found = false;
                        for (let retIndex in ret[index]) {
                            if (ret[index].hasOwnProperty(retIndex)) {
                                if (ret[index][retIndex].answer === answerObject['answers'][index]["answer"]) {
                                    ret[index][retIndex].count += 1;
                                    found = true;
                                }
                            }
                        }
                        if (!found) {
                            let ao = {};
                            ao.answer = answerObject['answers'][index]["answer"];
                            ao.count = 1;
                            ret[index].push(ao);
                        }
                    }
...
AiiaTrivia.prototype.getCorrectUsers = function (validAnswer) {
    ...
    for (let index in validObject) {
        if (validObject.hasOwnProperty(index)) {
            if (validObject[index]['ignore'] === false) {
    ...
    for (let i = 1; i < this.AddressCount + 1; i++) {
        ...
        for (let index in responses) {
            if (responses.hasOwnProperty(index)) {
                let isValid = true;
                let answerObject = JSON.parse(responses[index]['response']);
                for (let index in validIndex) {
                    if (validObject[validIndex[index]]['answer'] !== answerObject['answers'][validIndex[index]]['answer']) {
                        isValid = false;
                        break
                    }
                }
