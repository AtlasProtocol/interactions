...
let reservoir = function (array, m) {
    let result = [];
    let n = array.length;
...
let AIIA = function () {
    NASNanoSignup.call(this);
    LocalContractStorage.defineProperty(this, '_isSettled');
    LocalContractStorage.defineProperty(this, '_winners');
    LocalContractStorage.defineProperty(this, '_luckyList');
    this.TIEType = 'AIIA1.0.1';
};

(function () {
    let Super = function () {
    };
    Super.prototype = NASNanoSignup.prototype;
    AIIA.prototype = new Super();
})();

AIIA.prototype._checkNotSettled = function () {
    if (this._isSettled === true) {
        throw new Error('game settled');
    }
};

AIIA.prototype._checkSettled = function () {
    if (this._isSettled !== true) {
        throw new Error('game not settled');
    }
};

AIIA.prototype._checkInteractPreCondition = function (address) {
    this._checkNotSettled();
    this._checkInteracted(address);
};

AIIA.prototype._settlement = function (validAnswer) {

};

AIIA.prototype.interact = function (response, preCondition, variables) {
    throw new Error('interact not allowed');
};

AIIA.prototype.settle = function (validAnswer, number) {
    this._checkAuthorization();
    this._checkNotSettled();

    this._settlement(validAnswer);

    let winnerList = JSON.parse(this.getCorrectUsers(validAnswer));
    let count = number;

    if (count !== undefined && count < winnerList.length) {
        let ret = math.reservoir(winnerList, count);
        this._luckyList = JSON.stringify(ret);

    } else {
        this._luckyList = JSON.stringify(winnerList);
    }

    this._winners = JSON.stringify(winnerList);
    this._isSettled = true;
};

AIIA.prototype.getCorrectUsers = function (validAnswer) {
    return JSON.stringify(JSON.parse(this.getAddressWithResponse(validAnswer)) || [])
};

AIIA.prototype.getWinners = function () {
    this._checkSettled();
    return this._winners
};

AIIA.prototype.getLuckyList = function () {
    this._checkSettled();
    return this._luckyList
};

AIIA.prototype.isSettled = function () {
    return this._isSettled === true
};

const long = '{"answer":"1","index":0}';
const short = '{"answer":"0","index":0}';

let AIIABET = function() {
  AIIA.call(this);
  LocalContractStorage.defineProperty(this, '_btcPrice');
  LocalContractStorage.defineProperty(this, '_ratio');
  LocalContractStorage.defineProperty(this, '_result');
  this.TIEType = 'AIIABet1.0.1';
};

(function() {
  let Super = function() {};
  Super.prototype = AIIA.prototype;
  AIIABET.prototype = new Super();
})();

AIIABET.prototype.setTIE = function(tie, signature) {
  this._checkAuthorization(signature);
  if (typeof tie !== 'string') {
    throw new Error('tie has to be a string');
  }
  ...
  try {
    this._btcPrice = parsedTie['content'][0]['btcPrice'];
  } catch (e) {
    throw new Error('invalid format of tie');
  }
...
AIIABET.prototype.getBTCPrice = function() {
  return this._btcPrice;
};

AIIABET.prototype.getBetStatus = function() {
  let bullList = JSON.parse(this.getAddressWithResponse(long)) || [];
  let bearList = JSON.parse(this.getAddressWithResponse(short)) || [];

  return JSON.stringify({
    BullNumber: bullList.length,
    BearNumber: bearList.length,
  });
};

AIIABET.prototype.getBetResultList = function() {
  this._checkSettled();

  return {
    Winners: this._winners,
    Ratio: this._ratio + 1,
    IsBull: this._result === long,
  };
};

AIIA.prototype.getCorrectUsers = function(validAnswer) {
  let result = short;
  if (validAnswer > this._btcPrice) {
    result = long;
  }
  return JSON.stringify(JSON.parse(this.getAddressWithResponse(result)) || []);
};
...
AIIA.prototype._settlement = function(validAnswer) {
  ...
  if (validAnswer > this._btcPrice) {
    this._result = long;
    this._ratio = bullList.length === 0 ? 0 : bearList.length / bullList.length;
  } else {
    this._result = short;
    this._ratio = bearList.length === 0 ? 0 : bullList.length / bearList.length;
  }
};

module.exports = AIIABET;
